...
public class Marker {
  ...
  public Marker(String name, int row, int col) {
    this(name, row, col, 1, 1);
  }

  public Marker(int row, int col, int width, int height) {
    this(row + "," + col, row, col, width, height);
  }

  public Marker(String name, int row, int col, int width, int height) {
    setName(name);
    setRow(row);
    setCol(col);
    setWidth(width);
