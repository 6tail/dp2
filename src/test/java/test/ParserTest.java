...
public class ParserTest {
   ...
   * 模板文件
   */
  private final File templateFile = new File(getClass().getResource("/").getPath(), String.format("template.%s", TYPE));

  /**
   * 输出文件
   */
  private final File saveFile = new File(getClass().getResource("/").getPath(), String.format("save.%s", TYPE));
   ...
   */
  @Test
  public void writeLargeData() throws Exception {
    IParser parser = ParserFactory.getParser("xlsx");
    IWriter writer = parser.getWriter();
    writer.load();
    for (int i = 0; i < 1000000; i++) {
      writer.write(i, 0, new Node((String) null, String.valueOf(i)));
    }
    writer.save(saveFile);
  }
}
